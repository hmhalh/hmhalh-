...
def default_data():
    return {
        "account": None,           # يحتوي على اسم الجلسة ومعلومات الحساب
        "session_file": None,       # اسم ملف الجلسة المحدد
        "groups": [],
        "messages": {},
        "selected_groups": [],
        ...
        "delay": 60,
        "running": False,
        "limit_mode": "unlimited", # "unlimited" أو "custom"
        "max_messages": 0,         # العدد المستهدف
        "sent_count": 0            # عدد الرسائل التي تم إرسالها
    }
...
def load_data():
    ...
    try:
        ...
        try:
            broken = DATA_FILE.with_name("data_broken.json")
            if DATA_FILE.exists():
...
# =========================================================
# مهمة النشر التلقائي (إرسال سويّة لكل الكروبات دفعة واحدة)
# =========================================================
    ...
    while True:
        try:
            if data.get("running", False):
                ...
                msg_text = data.get("messages", {}).get(str(msg_id))
                ...
                if limit_mode == "custom" and sent_cnt >= max_msgs:
                    ...
                    # دالة فرعية لإرسال الرسالة لكروب واحد (لتنفيذها بشكل متزامن)
                    async def send_to_single_group(group):
                        ...
                        try:
                            ...
                            print(f"❌ خطأ بالإرسال إلى {group}: {e_send}")

                    # إرسال الرسائل لجميع الكروبات دفعة واحدة في نفس اللحظة
                    await asyncio.gather(*(send_to_single_group(group) for group in selected_groups))

                    # زيادة عداد الإرسال بمقدار عدد الكروبات التي تم النشر لها دفعة واحدة
                    data["sent_count"] = data.get("sent_count", 0) + len(selected_groups)
                    save_data(data)
...
async def set_session(update, context):
    query = update.callback_query
    session_stem = query.data.split(":", 1)[1]
...
# =========================================================
# معالج الأزرار الشفافة (Callback Queries)
# =========================================================
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cb == "home":
        ...
        await change_delay(update, context)
    elif cb == "status":
