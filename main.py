...
# =========================================================
# مهمة النشر التلقائي (مع الانضمام التلقائي)
# =========================================================
                ...
                if limit_mode == "custom" and sent_cnt >= max_msgs:
                    ...
                    for group in selected_groups:
                        if not data.get("running", False):
                            break
                        try:
                            # 1. محاولة الانضمام للكروب أولاً
                            try:
                                await client(JoinChannelRequest(group))
                                print(f"📥 تم الانضمام (أو التأكد من الانضمام) إلى: {group}")
                                await asyncio.sleep(3)  # انتظار بسيط لتجنب حظر التليجرام
                            except Exception as e_join:
                                print(f"⚠️ تنبيه أثناء الانضمام لـ {group}: {e_join}")
...
async def process_phone(update, phone):
    ...
    try:
        ...
        await update.message.reply_text("❌ رقم الهاتف غير صحيح.", reply_markup=back_keyboard())
    except Exception as e:
...
async def process_code(update, code):
    ...
    try:
        ...
        await update.message.reply_text("❌ الكود غير صحيح.", reply_markup=back_keyboard())
    except Exception as e:
        await update.message.reply_text(f"❌ خطأ:\n{e}", reply_markup=back_keyboard())
